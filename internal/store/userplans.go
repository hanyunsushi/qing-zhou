...
func enqueuePlanBucket(tx txLike, userID int64, username string, pkg *Package, orderID, now int64) error {
	...
	_, err = insertBucket(tx, &Bucket{
		...
		EdgeRequestLimit: pkg.EdgeRequestLimit,
...
func (b *Bucket) HasEdgeQuota() bool {
	return b.HasEdgeQuotaAt(time.Now().Unix())
}

func (b *Bucket) HasEdgeQuotaAt(now int64) bool {
	if b.EdgeRequestLimit == 0 || b.EdgeUsageDay != edgeUsageDayAt(now) {
		return true
	}
